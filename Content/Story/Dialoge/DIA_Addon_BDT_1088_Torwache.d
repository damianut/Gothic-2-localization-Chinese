...
{
	AI_Output(self, other, "DIA_Addon_BDT_1088_Addon_Torwache_FirstWarn_06_00"); //停 下 ！

	if (MIS_Send_Buddler == LOG_SUCCESS)
	{
		// kairo: aioutput-string des spielers gab es doppelt. umbenannt!

		AI_Output(other, self, "DIA_Addon_BDT_1088_Torwache_FirstWarn_15_01"); //别 挡 我 的 路 ， 卫 兵 。
		AI_Output(self, other, "DIA_Addon_BDT_10018_Torwache_FirstWarn_06_02"); //啊 ， 好 吧 ， 你 可 以 过 去 。
	...
	&& (MIS_Send_Buddler != LOG_SUCCESS))
	{
		...
		{
			AI_Output(self, other, "DIA_Addon_BDT_1088_Addon_Torwache_FirstWarn_06_03"); //只 要 托 鲁 斯 还 没 给 你 红 石 头 ， 这 里 就 不 欢 迎 你 。
			Torwache_Greetings = 1;
		}
		else if (Torwache_Greetings == 1)
		{
			AI_Output(self, other, "DIA_Addon_BDT_1088_Addon_Torwache_FirstWarn_06_04"); //你 没 听 明 白 我 的 话 吗 ？ （ 强 硬 的 ） 走 开 ！
			Torwache_Greetings = 2;
		}
		else
		{
			AI_Output(self, other, "DIA_Addon_BDT_1088_Addon_Torwache_FirstWarn_06_05"); //（ 强 硬 的 ） 走 开 ！
		};
	}
	else if (MIS_Send_Buddler == LOG_RUNNING)
	{
		AI_Output(self, other, "DIA_Addon_BDT_1088_Addon_Torwache_FirstWarn_06_06"); //嘿 ， 只 要 托 鲁 斯 还 没 有 发 出 放 行 信 号 ， 你 就 不 能 通 过 。
	};
...
{
	AI_Output(self, other, "DIA_Addon_BDT_1088_Addon_Torwache_SecondWarn_06_00"); //好 吧 ， 保 持 冷 静 。 你 还 有 最 后 一 次 机 会 。 马 上 回 去 ， 或 者 死 … …
...
func void DIA_BDT_1088_Addon_Torwache_Attack_Info()
{
	...
	AI_Output(self, other, "DIA_Addon_BDT_1088_Addon_Torwache_Attack_06_00"); //（ 叹 息 ） 你 以 为 我 在 开 玩 笑 ？ 我 会 让 你 看 看 … …
